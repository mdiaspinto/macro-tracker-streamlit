...
A small nutrition **data app**: import foods, log what you eat, and analyse your
daily calories and macronutrients. It is a Python/Streamlit port of a personal
macro-tracking project, restructured so the data logic is cleanly separated,
unit-tested, containerised, and reproducible.
...
- **Filter & aggregate** the log — by date range and by *eaten vs. what-if* — into
  daily calorie and macro series.
- **Visualise** calories and protein/carbs/fat per day with interactive Plotly charts.
- **Estimate targets** with the Mifflin-St Jeor equation (BMR → activity → TDEE,
  then a lean/cut and bulk/gain split).
...
| `macro_tracker/nutrition.py` | Calorie & target math (Mifflin-St Jeor) | `tests/test_nutrition.py` |
| `app.py` | Streamlit UI | — |
